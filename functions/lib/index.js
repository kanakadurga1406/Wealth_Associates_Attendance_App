 ...
 */
exports.markAttendance = functions.https.onCall(async (request) => {
    ...
    if (typeof latitude !== "number" || typeof longitude !== "number" || !type || !["checkIn", "checkOut"].includes(type)) {
        throw new functions.https.HttpsError("invalid-argument", "Missing or invalid parameters: latitude (number), longitude (number), type ('checkIn' | 'checkOut')");
    }
    try {
        ...
        if (officeQuery.empty) {
            throw new functions.https.HttpsError("failed-precondition", "Your office location settings have not been configured by the Admin.");
        }
        ...
        throw new functions.https.HttpsError("internal", error.message || "An error occurred while marking attendance.");
    }
});
/**
 ...
 */
exports.triggerAutoMarkAbsent = functions.https.onCall(async (request) => {
...
exports.createUserAccount = functions.https.onCall(async (request) => {
    ...
    if (!email || !password || !name || !role || !["ADMIN", "EMPLOYEE"].includes(role)) {
        throw new functions.https.HttpsError("invalid-argument", "Missing or invalid parameters: email, password, name, role ('ADMIN' | 'EMPLOYEE')");
    }
    try {
